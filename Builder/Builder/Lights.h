...
// Funkcje
	...
	void updateCGlightParams()
	{
		// ustawienie pozycji kamery w shaderze
			...
			cgSetParameter3f(terrainEyePosition, kamera.GetCameraPos().posX, kamera.GetCameraPos().posY, kamera.GetCameraPos().posZ);
			cgSetParameter3f(liquidEyePosition, kamera.GetCameraPos().posX, kamera.GetCameraPos().posY, kamera.GetCameraPos().posZ);
	...
	void lightKeyManager(char key, void *object, float &x, float &y, float &z, float &sizeX, float &sizeY, float &sizeZ)
	{
			switch( key )
			{
				...
				case 'L':
				{
					...
					// pobranie nowych ustawien

						light->ambient[0] =		(float)GetDlgItemInt(lighting, IDC_AMB_R, FALSE, FALSE) / 255.f;
						light->ambient[1] =		(float)GetDlgItemInt(lighting, IDC_AMB_G, FALSE, FALSE) / 255.f;
						light->ambient[2] =		(float)GetDlgItemInt(lighting, IDC_AMB_B, FALSE, FALSE) / 255.f;

						light->diff[0] =		(float)GetDlgItemInt(lighting, IDC_DIFF_R, FALSE, FALSE) / 255.f;
						light->diff[1] =		(float)GetDlgItemInt(lighting, IDC_DIFF_G, FALSE, FALSE) / 255.f;
						light->diff[2] =		(float)GetDlgItemInt(lighting, IDC_DIFF_B, FALSE, FALSE) / 255.f;

					// odswiezenie w OpenGL ustawien swiatla
	...
	void lightText(float &x, float &y, float &z, float &sizeX, float &sizeY, float &sizeZ, void *object)
	{
		...
		// tablice, ktore beda zawierac informacje
			...
			sprintf_s( CurrentAction, "Action : Moving light" ); 
	...
	{
		if( !objectOperator.removeSpecificObjects(LIGHT_BULB_NAME) )
		{
			MessageBox(g_hwnd, "There are no lights on the map.", "Error!", MB_OK);
		}
		else
			updateCGlightParams();
	}
	...
	void placeLight()
	{
		...
		{
			Vector3 wektor;
			wektor = GetOGLPos(g_mouseX, g_mouseY);
		...
		// nowe swiatlo
			...
			for(int i = 0; i < 3; i++)
			{
				value = GetDlgItemInt(lighting, index++, NULL, FALSE);
				newLight->ambient[i] = (float)value / 255.f;
			}
			for(int i = 0; i < 3; i++)
			{
				value = GetDlgItemInt(lighting, index++, NULL, FALSE);
				newLight->diff[i] = (float)value / 255.f;
			}

			newLight->ambient[3] = newLight->diff[3] =  1.f; //newLight->specular[3] =
		...
		// pozycjonowanie
			...
			float lightPosition[] = { wektor.posX, wektor.posY, wektor.posZ, 1.f };
		...
		// wlacz swiatelko
			...
			ObjectOperator::Object newObject(wektor.posX, wektor.posY, wektor.posZ, 

							  // rozmiary hitbox'a wokol swiatelka
							  lightBulb.sizeX*MAP_SCALE, lightBulb.sizeY*MAP_SCALE, lightBulb.sizeZ*MAP_SCALE, 
		...
		// inkrementacja licznika swiatel
		...
		// aktualizacja parametrow oswietlenia CG
			...
			MessageBox(g_hwnd, "Maximum light count has been exceeded.", "Error!", MB_OK);
	}
//
// --------------------------------------------
	...
	void saveLights(char *filename)
	{
		...
		// przeszukiwanie wektora obiektow w poszukiwaniu obiektow detalu
			...
			{
			// znaleziono swiatelko!

				if( objectOperator.objectArray[obj].GetType() == LIGHT_BULB_NAME )
				{
				...
				// pobranie obiektu detalu
					...
					LightSource *light = reinterpret_cast <LightSource*> ( objectOperator.objectArray[obj].GetObject() );
			...
			while( !file.eof() )
			{
				...
				// pobranie licznika cieczy
					...
					// pozycjonowanie
						...
						float lightPosition[] = { position.posX, position.posY, position.posZ, 1.f };
						glLightfv(newLight->ID, GL_POSITION, lightPosition);
