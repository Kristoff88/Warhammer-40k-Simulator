...
******************************************************/
	...
	void GUI_DataManager::AdjustGUIToClientArea (HWND const &hwnd) throw( std::string )
	{
		...
		if( !GetWindowRect(hwnd, &clientArea) )
		{
		...
		GUIborder.top    = clientArea.bottom;
		GUIborder.bottom = clientArea.top;

		GUIborder.left  = clientArea.left;
		GUIborder.right = clientArea.right;
	}
//	
